...
const initial = {
  ...
  dayTimetables: {},
  completed: {},
  gym: { workouts: [], bodyWeight: [], templates: GYM_SEED, plan: JSON.parse(JSON.stringify(GYM_PLAN)), favorites: [], recent: [], custom: [], bmi: [], settings: { units: 'kg', dark: true, height: '' } }
};
function load() {
  try {
    ...
    d.habitDone = d.habitDone || {};
    d.gym = d.gym || { workouts: [], bodyWeight: [], templates: GYM_SEED, settings: { units: 'kg', dark: true } };
    ...
    d.gym.settings = d.gym.settings || { units: 'kg', dark: true };
    d.gym.settings.dark = true;
...
function App() {
  ...
  const endDragRow = () => { dragFromRef.current = null; setDragFrom(null); window.removeEventListener('pointermove', onDragMove); window.removeEventListener('pointerup', endDragRow); };
  const onDragMove = (e) => {
    const el = document.elementFromPoint(e.clientX, e.clientY);
    const row = el && el.closest('.schedule.editing');
    const to = row && Number(row.dataset.idx);
    if (to == null || to === dragFromRef.current) return;
    const from = dragFromRef.current;
    setDraft(prev => { const list = [...prev]; const [m] = list.splice(from, 1); list.splice(to, 0, m); return list; });
    dragFromRef.current = to;
    setDragFrom(to);
  };
  const startDragRow = (e, i) => { e.preventDefault(); dragFromRef.current = i; setDragFrom(i); window.addEventListener('pointermove', onDragMove); window.addEventListener('pointerup', endDragRow); };
