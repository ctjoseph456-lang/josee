...
function StudyLogForm({ log, onSave, onDelete }) {
  const [hours, setHours] = useState(() => log ? String(Math.floor(logMinutes(log) / 60)) : '');
  const [minutes, setMinutes] = useState(() => log ? String(Math.round(logMinutes(log)) % 60) : '');
  ...
  const save = (e) => { e.preventDefault(); const h = Math.max(0, Number(hours) || 0); const m = Math.max(0, Math.min(59, Number(minutes) || 0)); if (!h && !m) return; onSave(h, m, notes.trim()); setSaved(true); };
  return <form onSubmit={save} className="study-q"><h3 className="study-q-title">How long did you study?</h3><div className="hm-box"><input className="hours-box" type="number" min="0" step="1" inputMode="numeric" placeholder="0" value={hours} onChange={e => setHours(e.target.value)} /><span className="hm-colon">h</span><input className="hours-box min" type="number" min="0" max="59" step="1" inputMode="numeric" placeholder="00" value={minutes} onChange={e => setMinutes(e.target.value)} /><span className="hm-colon">min</span></div><p className="hours-hint">Total = {fmtHM(total)}</p><h3 className="study-q-title">What subjects and topics did you cover?</h3><textarea className="topics-box" rows="9" placeholder="Type everything you studied..." value={notes} onChange={e => setNotes(e.target.value)} /><div className="log-actions"><button>Save</button>{saved && <span className="saved">Saved for this day — update it anytime</span>}{saved && <button type="button" className="ghost del" onClick={onDelete}>Delete this day's log</button>}</div></form>;
}
...
function App() {
  ...
  const [subjForm, setSubjForm] = useState({ subject: '', hours: '', minutes: '' });
  ...
  const weeklyMinutes = weekDays.map(d => Math.round((data.sessions.filter(s => s.date === d).reduce((n, s) => n + Number(s.minutes), 0)) + logMinutes(data.studyLog?.[d])));
  const saveLog = (h, m, notes) => update({ ...data, studyLog: { ...data.studyLog, [date]: { hours: h, minutes: m, notes } } });
  const deleteLog = () => { const l = { ...data.studyLog }; delete l[date]; update({ ...data, studyLog: l }); };
  ...
  const addSubject = (e) => { e.preventDefault(); if (!subjForm.subject.trim()) return; const m = Math.max(0, (Number(subjForm.hours) || 0) * 60 + (Number(subjForm.minutes) || 0)); if (!m) return; update({ ...data, subjectLog: [...(data.subjectLog || []), { id: crypto.randomUUID(), date, subject: subjForm.subject.trim(), minutes: m }] }); setSubjForm({ subject: '', hours: '', minutes: '' }); };
  ...
  const studyLogEntries = Object.keys(data.studyLog || {}).map(ds => ({ date: ds, minutes: logMinutes(data.studyLog[ds]), notes: data.studyLog[ds].notes || '' })).sort((a, b) => (a.date < b.date ? 1 : -1));
  const subjectTotals = {};
  ...
  {page === 'Study' && <div className="timetable-layout"><Calendar month={calMonth} onMonth={shiftMonth} todayStr={today()} selected={date} off={data.dayOff} marks={studyMarks} onPick={pickDate} /><section className="panel"><div className="panel-head"><div><h2>Daily study log</h2><p className="muted">{weekdayFull} · IST</p></div></div><StudyLogForm key={date} log={data.studyLog?.[date]} onSave={saveLog} onDelete={deleteLog} /><div className="study-subject"><h3 className="study-q-title">Time by subject</h3><p className="muted">Break down today's study by subject.</p><form className="subj-form" onSubmit={addSubject}><input placeholder="Subject, e.g. Maths" value={subjForm.subject} onChange={e => setSubjForm({ ...subjForm, subject: e.target.value })} required /><input type="number" min="0" step="1" placeholder="Hrs" value={subjForm.hours} onChange={e => setSubjForm({ ...subjForm, hours: e.target.value })} /><input type="number" min="0" max="59" step="1" placeholder="Min" value={subjForm.minutes} onChange={e => setSubjForm({ ...subjForm, minutes: e.target.value })} /><button>Add</button></form><div className="subj-list">{(data.subjectLog || []).filter(x => x.date === date).map(x => <div className="subj-item" key={x.id}><strong>{x.subject}</strong><small>{fmtHM(x.minutes)}</small><button className="ghost del" onClick={() => deleteSubject(x.id)}>×</button></div>)}</div></div></section></div>}
  ...
  {page === 'Reports' && <><div className="stats"><Card title="Weekly study" value={fmtHM(weeklyTotal)} note="Rolling seven days" /><Card title="Daily average" value={fmtHM(weeklyTotal / 7)} note="Rolling seven days" /><Card title="Study records" value={data.sessions.length + (data.subjectLog || []).length} note="Logs + subject entries, all time" /></div><section className="panel"><div className="panel-head"><div><h2>Study rhythm</h2><p className="muted">Time logged during the last seven days</p></div></div><div className="bar-chart">{weeklyMinutes.map((v, i) => <div className="bar" key={weekDays[i]}><span>{fmtHM(v)}</span><i style={{ height: `${Math.max(4, v / chartMax * 100)}%` }} /><small>{WEEK[dateFromIST(weekDays[i]).getUTCDay()]}</small></div>)}</div></section><section className="panel"><div className="panel-head"><div><h2>Time by subject</h2><p className="muted">Total time per subject, all time</p></div></div>{subjectList.length ? <div className="subj-bars">{subjectList.map(s => <div className="subj-bar" key={s.subject}><span className="subj-name">{s.subject}</span><div className="subj-track"><i style={{ width: `${Math.round(s.minutes / subjMax * 100)}%` }} /></div><strong>{fmtHM(s.minutes)}</strong></div>)}</div> : <p className="muted">Add time by subject in the Study page to see a breakdown here.</p>}</section><section className="panel"><div className="panel-head"><div><h2>History of study reports</h2><p className="muted">{studyLogEntries.length} days logged</p></div></div>{studyLogEntries.length ? <div className="study-hist">{studyLogEntries.map(h => <div className="study-item" key={h.date}><div className="study-meta"><strong>{fmtHM(h.minutes)}</strong><small>{h.date}</small></div><p className="study-notes">{h.notes || '—'}</p><button className="ghost del" title="Delete this day's log" onClick={() => { if (confirm(`Delete the study log for ${h.date}?`)) { const l = { ...data.studyLog }; delete l[h.date]; update({ ...data, studyLog: l }); } }}>×</button></div>)}</div> : <p className="muted">No study logs yet — fill in the Study page.</p>}</section></>}
