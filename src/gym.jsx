...
export function Gym({ data, update }) {
  ...
  const deleteWorkout = (id) => { updateGym({ ...data.gym, workouts: data.gym.workouts.filter(w => w.id !== id) }); showToast('Workout removed'); };
  ...
  return <div className="gym">
    ...
    {view === 'Start Workout' && active && !summary && <>
      ...
      {active.exercises.length ? <section className="panel">
        <div className="panel-head"><div><h2>Today's sets</h2><p className="muted">Pre-filled from your last {DAY_LABELS[DAY_KEYS.indexOf(dayKeyOf(active.date))]} workout — edit and mark done as you go.</p></div></div>
        <div className="wt-wrap">
          <div className="wt-row wt-head" style={{ gridTemplateColumns: `190px repeat(${maxSets},1fr)` }}><span className="wt-ex">Exercise</span>{Array.from({ length: maxSets }, (_, i) => <span className="wt-set" key={i}>Set {i + 1}</span>)}</div>
          {active.exercises.map((ex, ei) => { const prev = prevWorkoutFor(data, active.date, ex.name) || []; const best = bestWeightBefore(data, ex.name, data.gym.workouts.length); return <div className="wt-row" data-exid={ex.id} key={ex.id} style={{ gridTemplateColumns: `190px repeat(${maxSets},1fr)` }}>
            <div className="wt-ex"><strong>{ex.name} <span className="ex-count">{ei + 1}/{active.exercises.length}</span></strong><small>{ex.muscle}{ex.equipment ? ` · ${ex.equipment}` : ''}{best ? ` · best ${dispW(best, units)} ${units}` : ''}{rest && rest.exId === ex.id ? ` · Rest ${restLeft}s` : ''}</small><div className="wt-actions"><select className="cat rest-select" value={rest && rest.exId === ex.id ? rest.total : 60} onChange={e => startRest(ex.id, Number(e.target.value))}>{REST_PRESETS.map(s => <option key={s} value={s}>{s}s rest</option>)}</select><button className="ghost" onClick={() => addSet(ex.id)}>+ Add set</button><button className="ghost del" onClick={() => deleteExercise(ex.id)}>Remove</button></div></div>
            {ex.sets.map((s, si) => { const pw = prev[si]; const w = Number(s.weight) || 0; const r = Number(s.reps) || 0; const isPR = w > 0 && w > best; const is1PR = w > 0 && r > 0 && epley(w, r) > best1RMBefore(data, ex.name, data.gym.workouts.length); return <div className={`wt-set${s.done ? ' done' : ''}`} key={s.id}>
              {pw ? <small className="wt-prev">{dispW(pw.weight, units)} kg × {pw.reps}</small> : <small className="wt-prev">&nbsp;</small>}
    ...
    {view === 'Start Workout' && !active && <div className="panel start-panel">
      <div className="panel-head"><div><h2>Start Workout</h2><p className="muted">Tap the button to begin — exercises come from your {DAY_LABELS[DAY_KEYS.indexOf(startDay)]} routine.</p></div></div>
      <div className="start-now-wrap"><button className="start-now" onClick={() => startFromDay(startDay)}>Start Workout</button><button className="ghost" onClick={startBlank}>Blank workout</button></div>
      {!planFor(startDay).length && <div className="rest-day"><strong>Rest day</strong><p className="muted">No exercises set for {DAY_LABELS[DAY_KEYS.indexOf(startDay)]}. Pick another day or set one up in Edit Workout / Routine.</p></div>}
      <div className="start-pick"><label className="log-field"><span>Routine day</span><select className="cat" value={startDay} onChange={e => setStartDay(e.target.value)}>{DAY_KEYS.map(k => <option key={k} value={k}>{DAY_LABELS[DAY_KEYS.indexOf(k)]}{k === todayDay ? ' · today' : ''}{planFor(k).length ? ` (${planFor(k).length})` : ' — Rest'}</option>)}</select></label></div>
      {planFor(startDay).length ? <><div className="plan-ex-list today">{planFor(startDay).map((en, i) => <div className="plan-ex" key={i}><span className="set-num">{i + 1}</span><strong>{en.name}</strong><small className="muted">{findEx(data, en.name)?.primary || ''} · {en.sets.length} sets</small></div>)}</div></> : null}
    </div>}
    ...
    {view === 'Reports' && <>
      ...
      <section className="panel"><div className="panel-head"><div><h2>Workout history</h2><p className="muted">{ws.length} saved session{ws.length === 1 ? '' : 's'} — every set, rep, and weight</p></div></div>
        {ws.length ? <div className="hist-list">{ws.map(w => { const hVol = workoutVolume(w); const hSets = (w.exercises || []).reduce((t, ex) => t + (ex.sets || []).length, 0); const hReps = (w.exercises || []).reduce((t, ex) => t + (ex.sets || []).reduce((s, x) => s + (Number(x.reps) || 0), 0), 0); return <div className="hist-item" key={w.id}>
          <div className="hist-head"><div><strong>{w.name || `Workout — ${w.date}`}</strong><small>{fmtDate(w.date)} · {DAY_LABELS[DAY_KEYS.indexOf(dayKeyOf(w.date))]} · {w.durationMin} min</small></div><button className="ghost del" onClick={() => deleteWorkout(w.id)}>Remove</button></div>
          <div className="hist-sum"><span>Exercises {w.exercises?.length || 0}</span><span>Sets {hSets}</span><span>Reps {hReps.toLocaleString()}</span><span>Volume {Math.round(hVol).toLocaleString()} {units}</span></div>
          {w.notes ? <p className="hist-notes">{w.notes}</p> : null}
          <div className="hist-exes">{(w.exercises || []).map((ex, i) => <div className="hist-ex" key={i}><strong>{ex.name}</strong><small>{ex.muscle || ''}</small><div className="hist-sets">{(ex.sets || []).map((s, si) => { const wgt = Number(s.weight) || 0; const rep = Number(s.reps) || 0; return <span key={si}>{wgt && rep ? `${dispW(wgt, units)} kg × ${rep}` : wgt ? `${dispW(wgt, units)} kg` : rep ? `${rep} reps` : '—'}</span>; })}</div></div>)}</div>
        </div>; })}</div> : <p className="muted">No saved workouts yet — finish a workout first.</p>}
      </section>
    </>}
